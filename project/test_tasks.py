...
class TestTasks(unittest.TestCase):
    ...
    def test_user_cannot_delete_tasks_that_are_not_created_by_them(self):
        self.create_user('mammad', 'mammad@ammma.cmma', 'python')
        self.login('mammad', 'python')
        self.app.get('tasks', follow_redirects=True)
        self.create_task()
        self.logout()
        self.create_user('mammadism', 'mammad@ammmaaa.cmma', 'pythonEE')
        self.login('mammadism', 'pythonEE')
        response = self.app.get("delete/1", follow_redirects=True)
        self.assertIn(
            b"YOU can't",
            response.data
